(function(window, document, undefined) {
    ...
    var defaults = {
        messages: {
            ...
            valid_credit_card: '%s must contain a valid credit card number.',
            is_file_type: '%s must contain only %s files.',
            valid_url: '%s must contain a valid URL.',
            password_strength:"%s must be minimum 8 characters.",
            expiryMonth : "Enter the correct expiry month",
            expiryYear : "Enter the correct expiry year",
            cvv : "Enter the correct CVV",
    ...
    var ruleRegex = /^(.+?)\[(.+)\]$/,
        ...
        //passwordRegex = /^(((?=.*[A-Z])(?=.*[!@#$])(?=.*[0-9])(?=.*[a-z]).{8,16})|(\d){4})$/,
...
//            })(this);
    ...
    FormValidator.prototype._hooks = {
        ...
        password_strength: function(field) {
        ...
        expiryMonth : function checkExpiryMonth(field, matchName) {
            var dateNow = new Date();
            var year = "20" + this.form[matchName].value;
            if(dateNow.getMonth() >= field.value) {
                if(year <= dateNow.getFullYear())
                    return false;
            }
            return true;
        },
        expiryYear : function checkExpiryYear(field, matchName) {
            var dateNow = new Date();
            var year = "20" + field.value;
            var month = this.form[matchName];
            if(dateNow.getFullYear() > year) {
                return false;
            } else if(month.value < dateNow.getMonth() && year == dateNow.getFullYear()) {
                return false;
            }
            return true;
        },
