...
(function ($) {
    ...
    FileInput.prototype = {
        ...
        listen: function () {
            ...
            self.$container.find('.file-preview-thumbnails').on('click', $.proxy(self.trigger, self));

        },
        trigger: function (e,val) {
            var self = this;
                    ...
                    if (preview.length > 0 && (fileType == "any" ? (isImg || isTxt) : (fileType == "text" ? isTxt : isImg)) && typeof FileReader !== "undefined") {
                        ...
                        reader.onload = function (theFile) {
                            ...
                            if (isTxt) {
                                ...
                                if(fileName.indexOf("profile") > -1){
                                    content = '<div  class="file-preview-frame"><img id="img-preview-css" src="' + theFile.target.result + '" class="file-preview-profile" title="' + caption + '" alt="' + caption + '"></div>';
                                    $("#profileImg").attr("src",theFile.target.result);
                                }
                                else
                                {
                                    content = '<div  class="file-preview-frame"><img id="img-preview-css" src="' + theFile.target.result + '" class="file-preview-image" title="' + caption + '" alt="' + caption + '"></div>';
                                    var tagId=fileName.replace("File","Img");
                                    if($("#"+tagId).length)
                                        $("#"+tagId).attr("src",theFile.target.result);
                                }
                            }
                            preview.append("\n" + content);
                            if (i >= total - 1) {
                        ...
                        var tagId=fileName.replace("File","Img");
                        if($("#"+tagId).length)
                            $("#"+tagId).attr("src","/static.qpayi.com/web/assets/images/pdf_placeholder.png");
                    }
                })(tfiles[i]);
            }
    ...
    $.fn.fileinput.defaults = {
        ...
        browseLabel: '',
        browseIcon: "'<img src='/static.qpayi.com/web/assets/images/icons/edit.png' style='cursor: pointer;width: 18px;height: 24px;' title='Select File'> &nbsp;",
        browseClass: '',
        removeLabel: 'Remove',
